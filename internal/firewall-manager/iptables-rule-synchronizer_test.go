...
type TestIPTablesRule struct {
	ID      string
	Content string
	SpecVal []string
}

func (r *TestIPTablesRule) RuleID() string {
	if r == nil {
		return ""
	}
	return r.ID
...
func (r *TestIPTablesRule) String() string {
	if r == nil {
		return ""
	}
	return r.Content
...
func (r *TestIPTablesRule) Spec() []string {
	if r == nil {
		return []string{}
	}
	return r.SpecVal
...
func testParser(spec []string, protocol iptables.Protocol) (*TestIPTablesRule, bool) {
	if len(spec) < 2 {
		return nil, false
	}
...
func createTestSynchronizer(mockIPT iptables.IPTables) *IPTablesRuleSynchronizer[*TestIPTablesRule] {
	sync := NewIPTablesRuleSynchronizer[*TestIPTablesRule](mockIPT)
...
func TestIPTablesRuleSynchronizer_Sync(t *testing.T) {
	...
	t.Run("removes multiple rules with same ID when present is false", func(t *testing.T) {
		...
		customParser := func(spec []string, protocol iptables.Protocol) (*TestIPTablesRule, bool) {
			if len(spec) < 4 {
				return nil, false
			}
	...
	t.Run("returns error when ListRules fails", func(t *testing.T) {
		...
		sync := createTestSynchronizer(mockIPT)
		sync.Rule = &TestIPTablesRule{ID: "test-rule-1"}
		sync.Present = true
...
func TestIPTablesRuleSynchronizer_RealWorldScenario(t *testing.T) {
	t.Run("manages SNAT rules with different source IPs", func(t *testing.T) {
		...
		snatParser := func(spec []string, protocol iptables.Protocol) (*TestIPTablesRule, bool) {
			if len(spec) < 8 {
				return nil, false
			}
