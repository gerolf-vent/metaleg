...
func NewManager(fwMask utils.FWMask, routeTableIDOffset uint32) (*Manager, error) {
	if fwMask.Size() <= 1 {
...
func (m *Manager) Cleanup() error {
	...
	routeTableIDMin := int(m.routeTableIDOffset)
	routeTableIDMax := int(uint(m.routeTableIDOffset) + m.fwMask.Size() - 1)

	for _, family := range []int{netlink.FAMILY_V4, netlink.FAMILY_V6} {
		// Cleanup netlink rules
		ruleCleaner := &RuleCleaner{
...
func (m *Manager) ReconcileNodeRoute(route *rm.NodeRoute, present bool) error {
	if route == nil {
		return nil
	}
	...
	if route.RuleCount > 0 && !route.IDAllocated {
		...
		var err error
		route.ID, err = m.idAllocator.Allocate()
		if err != nil {
		...
		route.FWMark = (uint32(route.ID) + 1) << uint32(m.fwMask.Shift())
		route.RouteTableID = m.routeTableIDOffset + uint32(route.ID)
	}
	...
	if route.RuleCount == 0 && route.IDAllocated {
		m.idAllocator.Release(route.ID)
		route.ID = 0
...
func (m *Manager) CleanupStaleNodeRoutes(routes map[string]*rm.NodeRoute) error {
	routeTableIDMin := int(m.routeTableIDOffset)
	routeTableIDMax := int(uint(m.routeTableIDOffset) + m.fwMask.Size() - 1)
