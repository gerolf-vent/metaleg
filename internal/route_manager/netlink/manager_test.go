...
func TestNewManager(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			manager, err := NewManager(tt.fwMask, tt.routeTableIDOffset)
...
func TestManager_Setup(t *testing.T) {
	...
	manager, err := NewManager(fwMask, 10000)
	if err != nil {
...
func TestManager_Cleanup(t *testing.T) {
	...
	fwMask := utils.FWMask(0xFF000000)
	manager, err := NewManager(fwMask, testTableOffset)
	if err != nil {
		t.Fatalf("Failed to create Manager: %v", err)
	}
...
func TestManager_ReconcileNodeRoute(t *testing.T) {
	requireNetlinkWritePrivileges(t)
	...
	manager, err := NewManager(fwMask, routeTableOffset)
	if err != nil {
		t.Fatalf("Failed to create Manager: %v", err)
	}
	...
	tests := []struct {
		name        string
		route       *rm.NodeRoute
		prepare     func(t *testing.T, route *rm.NodeRoute)
		verify      func(t *testing.T, route *rm.NodeRoute)
		expectError bool
		expectedErr string
		present     bool
	}{
...
func TestManager_CleanupStaleNodeRoutes(t *testing.T) {
	requireNetlinkWritePrivileges(t)
	...
	manager, err := NewManager(testFWMask, routeTableOffset)
	if err != nil {
		t.Fatalf("Failed to create Manager: %v", err)
	}

	testRoutes := map[string]*rm.NodeRoute{
		"node1": {
			Name:         "node1",
			IPv4:         testIPv4s[0],
			IPv6:         testIPv6s[0],
	...
	tests := []struct {
		name        string
		routes      map[string]*rm.NodeRoute
