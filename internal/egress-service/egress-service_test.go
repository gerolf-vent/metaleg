...
func TestEgressService_Start(t *testing.T) {
	t.Run("initializes service and runs reconciliation", func(t *testing.T) {
		es, mockFW, mockRM := createTestEgressService()
		...
		if !es.IsReady() {
			t.Error("expected service to be ready after start")
		}

		// Verify setup was called
		if mockFW.setupCalls < 1 {
			t.Errorf("expected at least 1 firewall setup call, got %d", mockFW.setupCalls)
		}
		if mockRM.setupCalls < 1 {
			t.Errorf("expected at least 1 route setup call, got %d", mockRM.setupCalls)
		}
		...
		// Verify reconciliation occurred (setup should be called multiple times due to ticker)
		if mockFW.setupCalls < 2 {
			t.Errorf("expected at least 2 firewall setup calls (initial + reconciliation), got %d", mockFW.setupCalls)
		}
		if mockRM.setupCalls < 2 {
			t.Errorf("expected at least 2 route setup calls (initial + reconciliation), got %d", mockRM.setupCalls)
		}
	})

	t.Run("handles firewall manager setup error", func(t *testing.T) {
		es, mockFW, _ := createTestEgressService()
		mockFW.setupErr = errors.New("firewall setup error")
		...
		if err == nil {
			t.Error("expected error from firewall manager setup")
		}
		if err.Error() != "failed to setup firewall manager: firewall setup error" {
			t.Errorf("expected firewall setup error, got %v", err)
		}

		if es.IsReady() {
			t.Error("expected service to not be ready after setup error")
		}
	})

	t.Run("handles route manager setup error", func(t *testing.T) {
		es, _, mockRM := createTestEgressService()
		mockRM.setupErr = errors.New("route setup error")
		...
		if err == nil {
			t.Error("expected error from route manager setup")
		}
		if err.Error() != "failed to setup route manager: route setup error" {
			t.Errorf("expected route setup error, got %v", err)
		}
