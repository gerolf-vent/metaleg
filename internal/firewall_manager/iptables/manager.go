...
func NewManager(nodeName string, fwMask uint32, excludeDstCIDRs []net.IPNet) (*Manager, error) {
	var err error
...
func (iptm *Manager) Cleanup() error {
	var errs []error
...
func (iptm *Manager) ReconcileEgressRule(rule *fm.EgressRule, present bool) error {
	if rule == nil {
		return nil
	}
	...
	for _, ipt := range []iptables.IPTables{iptm.ipt4, iptm.ipt6} {
		...
		rejectRuleSynchronizer := &RuleSynchronizer[*RejectRule]{
			ipt:     ipt,
		...
		rtMarkRuleSynchronizer := &RuleSynchronizer[*MarkRule]{
			ipt:     ipt,
		...
		snatRuleSynchronizer := &RuleSynchronizer[*SNATRule]{
			ipt:     ipt,
...
func (iptm *Manager) CleanupStaleEgressRules(rules map[string]*fm.EgressRule) error {
	var errs []error
	...
	for _, ipt := range []iptables.IPTables{iptm.ipt4, iptm.ipt6} {
		...
		rejectRuleCleaner := &RuleCleaner[*RejectRule]{
			ipt:             ipt,
			Parser:          ParseRejectRule,
			Table:           iptables.TableFilter,
			Chain:           iptablesRejectChainName,
		...
		rtMarkRuleCleaner := &RuleCleaner[*MarkRule]{
			ipt:             ipt,
			Parser:          ParseMarkRule,
			Table:           iptables.TableMangle,
			Chain:           iptablesRTMarkChainName,
		...
		snatRuleCleaner := &RuleCleaner[*SNATRule]{
			ipt:             ipt,
			Parser:          ParseSNATRule,
			Table:           iptables.TableNAT,
			Chain:           iptablesSNATChainName,
