...
func TestParseExcludeCIDRsRule_Valid(t *testing.T) {
	tests := []struct {
		name     string
		spec     []string
		protocol iptables.Protocol
		expected *ExcludeCIDRsRule
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, ok := ParseExcludeCIDRsRule(tt.spec, tt.protocol)
			if !ok {
				t.Fatalf("ParseExcludeCIDRsRule() failed, expected success")
			}
			if !reflect.DeepEqual(result, tt.expected) {
				t.Errorf("ParseExcludeCIDRsRule() = %+v, want %+v", result, tt.expected)
			}
		})
	}
}

func TestParseExcludeCIDRsRule_Invalid(t *testing.T) {
	tests := []struct {
		name     string
		spec     []string
		protocol iptables.Protocol
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, ok := ParseExcludeCIDRsRule(tt.spec, tt.protocol)
			if ok {
				t.Errorf("ParseExcludeCIDRsRule() succeeded with %+v, expected failure", result)
			}
		})
	}
}

func TestExcludeCIDRsRule_Spec(t *testing.T) {
	tests := []struct {
		name     string
		rule     *ExcludeCIDRsRule
		expected []string
	}{
		{
			name: "Simple rule",
...
func TestExcludeCIDRsRule_RoundTrip(t *testing.T) {
	tests := []struct {
		name string
		rule *ExcludeCIDRsRule
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			spec := tt.rule.Spec()
			parsed, ok := ParseExcludeCIDRsRule(spec, tt.rule.Protocol)
			if !ok {
