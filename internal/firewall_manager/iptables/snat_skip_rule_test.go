...
func TestParseSNATSkipRule_Valid(t *testing.T) {
	tests := []struct {
		name     string
		spec     []string
		expected *SNATSkipRule
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, ok := ParseSNATSkipRule(tt.spec)
			if !ok {
				t.Fatalf("ParseSNATSkipRule() failed, expected success")
			}
			if !reflect.DeepEqual(result, tt.expected) {
				t.Errorf("ParseSNATSkipRule() = %+v, want %+v", result, tt.expected)
			}
		})
	}
}

func TestParseSNATSkipRule_Invalid(t *testing.T) {
	tests := []struct {
		name string
		spec []string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, ok := ParseSNATSkipRule(tt.spec)
			if ok {
				t.Errorf("ParseSNATSkipRule() succeeded with %+v, expected failure", result)
			}
		})
	}
}

func TestSNATSkipRule_Spec(t *testing.T) {
	tests := []struct {
		name     string
		rule     *SNATSkipRule
		expected []string
...
func TestSNATSkipRule_RoundTrip(t *testing.T) {
	tests := []struct {
		name string
		rule *SNATSkipRule
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			parsed, ok := ParseSNATSkipRule(spec)
			if !ok {
